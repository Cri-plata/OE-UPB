...
## Completado el 2026-09-25 — P3 Calidad de contrato y documentación

- [x] **API-02 — Contrato de errores:** `presentation/errores.py` define `ErrorResponse` (`detail` como texto o `{codigo, mensaje}`); cada router protegido declara 401/403 y cada endpoint sus 400/404/409/413/500. `DELETE /api/usuarios/{id}` queda marcado como obsoleto (alias de `POST /desactivar`) y el contrato documenta el patrón `@upb.edu.co`. Pruebas en `test_calidad_p3.py`.
- [x] **PRG-01 — Programas por clave normalizada:** `application/programas.py` compara sin mayúsculas, tildes ni espacios repetidos en la audiencia de publicaciones y en las asignaciones; la carga y el directorio limpian espacios del nombre. Un programa asignado que deja de observarse se conserva, no bloquea la edición y se informa en `programas_sin_datos`; la pantalla de cuentas lo muestra como "sin datos actuales".
- [x] **ANA-02 — Criterios de alertas:** solo momentos de seguimiento (M1/M5), por programa y momento, con la taxonomía de ADR-016; muestra mínima de 5; severidad `media` por debajo del 70 % y `alta` por debajo del 50 %; alerta de texto con al menos 5 textos y 3 negativos. `tipo` y `severidad` son enumerados en el contrato.
- [x] **DOC-03 — Modelo y catálogo de requisitos:** diagrama ER completo (auditorías, eventos de eliminación y columnas de publicaciones); prioridad propuesta para los 81 RF/RNF (38 Alta, 35 Media, 8 Baja); `Requerimiento Ligado` corregidos (RF-43, RF-44, RF-45, RF-69, RNF-02, RNF-03, RNF-06); criterios verificables propuestos para RNF-01 a RNF-06. La validación de producto sigue en PRD-01.
- [x] **FIX — Documento en el formulario de cuentas:** el frontend exigía solo dígitos, en contra de ADR-017; ahora acepta letras, números y separadores.